...
class GeolocationBadLocalFileArchive(GeolocationLocalFileArchive):
    location = os.path.join(
        ...
        '../core/management/commands/tests/missing-database-file.tar.gz'
    )
...
def test_handles_remote_invalid_archive(settings):
    ...
    with pytest.raises(tarfile.ReadError):
        geolocation_archive.decompress(
            ...
            destination=os.path.join(settings.GEOIP_PATH, 'test')
        )
...
@patch(
    'core.management.commands.download_geolocation_data.'
    'GeolocationArchiveNegotiator'
)
